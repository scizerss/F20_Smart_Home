...
void UART_Receive_String(uint8_t* ptr)       //fn that receives an empty array and populates the array through calling the fn (UART_Receive())
   {                                          //it has no return
	uint8_t i;
	ptr[i] = UART_Receive();                //receive 1st char
	while(ptr[i] != '#')                    //we ask the user to terminate each string he enters with '#'
	{                                        //we loop till we find the '#' and receive all characters and add them in the array
		i++;
		ptr[i] = UART_Receive();
	}
	ptr[i] = '\0';                           //after populating the array, we add a null
}